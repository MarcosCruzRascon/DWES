...
function respuestasCorrectas($codExamen)
{
    try {
        $conexion = connectDB();
        $sql = "SELECT respuestaCorrecta FROM examenes.examenes_tiene_preguntas as a inner join preguntas as b 
...
function obtieneUsuario($usuario)
{
    try {
        $conexion = connectDB();
        $sql = "SELECT * FROM examenes.usuarios where idusuarios=?;";
        $consulta = $conexion->prepare($sql);
...
function examenRealizado($usuario, $examen){
    ...
    try {
        $conexion = connectDB();
        $sql = "SELECT * FROM examenes.usuarios_hace_examenes where usuarios_idusuarios = ? and examenes_codExamen= ?;";
        $consulta = $conexion->prepare($sql);
        $consulta->bindParam(1, $idusuario);
        $consulta->bindParam(2, $codExamen);
        $consulta->execute();
...
function obtieneInfoExamen($examen){
    ...
    try {
        $conexion = connectDB();
        $sql = "SELECT * FROM examenes.examenes where codExamen= ?;";
        $consulta = $conexion->prepare($sql);
        $consulta->bindParam(1, $codExamen);
        $consulta->execute();
        $fila = $consulta->fetchObject();
